...
Here is the link to the dataset: https://www.kaggle.com/mterzolo/lego-sets/download
...
## using the testing data set to generate predictions
...
```
#make sure we talk about in-bag vs. out of bag predictions
...
#### Evaluating Lasso train and test metrics
```{r}
metrics(results_train_lasso, X1, true) #replace X1 with preds
# fix this, for some reason preds is X1

metrics(results_test_lasso, preds, true)

```

#### Plotting the lasso train results
```{r}
...
#### Plotting the lasso test results
```{r}
ggplot(results_test_lasso, 
       aes(x = true, y = X1)) +
  geom_point(alpha = 1/2, size = 4) +
  geom_smooth(color = "red")
```

## Need to change all the '###' to '####'

-- I used this to show the issue with ages
```{r}
